...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, out_seq_len=None):
        ...
        outputs = torch.zeros(batch_size, seq_len, out_size).to(self.device)
        _, hidden = self.encoder(source)  
        sos = source[:,0].unsqueeze(1)           
        #sos = source[:,0]
        for i in range(seq_len):
            pred, hidden = self.decoder(sos, hidden)
            outputs[:,i,:] = pred
            #sos = source[:,i].unsqueeze(1)
            sos = pred.argmax(1).unsqueeze(1)  
        #############################################################################
        #                              END OF YOUR CODE                             #
        #############################################################################
        return outputs
